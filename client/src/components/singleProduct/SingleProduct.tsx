...
import { CiHeart } from 'react-icons/ci';
import { BiPencil, BiTrash } from 'react-icons/bi';
import { AiFillHeart } from 'react-icons/ai'; // Import full heart icon for favorite functionality
import { useNavigate } from 'react-router-dom';
import { addToCart, addToFavorites, removeFromFavorites } from '../../slices/userSlice';
import { deleteProduct } from '../../slices/productSlice';
import { SingleProductProps } from '../../common/interfaces';
...
const SingleProduct: React.FC<SingleProductProps> = ({ product }) => {
    ...
    const userRole = useSelector((state: RootState) => state.user.userDetails.role); // Get user role (admin or user)
    const favorites = useSelector((state: RootState) => state.user.favorites); // Access user's favorite products

    // Check if the current product is in the user's favorites list
    const isFavorite = favorites.some((fav: any) => fav === product._id || fav._id === product._id);

    // Handle adding product to cart, ensures user is authenticated
    const handleAddToCart = () => {
        if (isAuthenticated) {
            dispatch(addToCart(product._id) as any);
        } else {
    ...
    // Navigate to edit product page, only available for admin
    const handleEditProduct = () => {
        navigate(`/edit-product/${product._id}`);
    };

    // Handle product deletion, only available for admin
    const handleDeleteProduct = () => {
        dispatch(deleteProduct(product._id) as any); // Dispatch delete action with the product's ID
    };
    ...
    return (
        <div className="single-product">
            ...
            <div className="product-actions">
                ...
                {/* Only display edit and delete options if user is an admin */}
                {userRole === 'admin' && (
                    <>
                        <button onClick={handleEditProduct} className="edit-product-btn">
                            <BiPencil />
